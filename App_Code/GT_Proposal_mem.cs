...
/// </summary>
public class GT_Proposal_mem
{
    public string ref_id { get; private set; }
    ...
    public GT_Proposal_mem(string mem_id)
    {
        string sql = "select REF_NO, MEM_ID, MEM_TYPE, GENDER, to_char(DOB, 'yyyy/mm/dd') AS DOB , AGE, NAME, PP_NO, TITLE," +
                     " BASE_AMOUNT_USD, ENTERED_DATE, decode(MEM_TYPE, 'M', 'Main Life', 'S', 'Spouse', 'C', 'Child', 'O', 'Other'), " +
                     " decode(GENDER, 'M', 'Male', 'F', 'Female') from SLIC_NET.GT_MEM_DETAILS  WHERE MEM_ID = '" + mem_id.Trim() + "' ";
